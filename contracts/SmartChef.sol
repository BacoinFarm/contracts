...
// added functionnalities by the barcoin's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when BACOIN mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 bacoinReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake BACOIN tokens to TheBushV1
    ...
    // Withdraw BACOIN tokens from STAKING.
