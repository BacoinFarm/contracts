...
// MasterChefV2 is the master of Bacoin. He can make Bacoin and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function pendingBacoin(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accBacoinPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeBacoinTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function safeBacoinTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > bacoinBal) {
            transferSuccess = bacoin.transfer(_to, bacoinBal);
        } else {
            transferSuccess = bacoin.transfer(_to, _amount);
        }
		
        require(transferSuccess, "safeBacoinTransfer: transfer failed");
    }
    ...
    //Bacoin has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _bacoinPerBlock) public onlyOwner {
        massUpdatePools();
        bacoinPerBlock = _bacoinPerBlock;
		
		emit UpdateEmissionRate(msg.sender, _bacoinPerBlock);
    }
}
